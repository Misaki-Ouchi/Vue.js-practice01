...
Vue.createApp({
  ...
  methods: {
    ...
    selectFunc: function (e) {
      ...
      // 選択肢を非表示
      list.show[4] = false;
      this.cityShow = false // 市町村選択の場合の選択肢
      // 都道府県選択の場合
      if (list.branch === "pref") {
        if (value === "北海道・東北") {
    ...
    getCityData: async function () {
      ...
      // 都道府県コード取得
      let code = this.prefCode[num2]['code']
      if(code < 10 ){ code = "0" + code} // 一桁の場合前に0
      // 該当する市町村データ取得
      let res = await fetch(`https://www.land.mlit.go.jp/webland/api/CitySearch?area=${code}`)
